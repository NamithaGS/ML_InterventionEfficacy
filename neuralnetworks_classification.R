...
nn <- train(high_effectsize~mean_rxbaseline + epc_randomization_unit_ehr_imple + nos_pct1 + epc_randomization_unit_enrolled_ + epc_randomization_unit_has_resid,data=training, method ='nnet',linear.output=F,maxiter= 200 ,act.fct = "logistic"   )
nnbest <- nnet(high_effectsize~mean_rxbaseline + epc_randomization_unit_ehr_imple + nos_pct1 + epc_randomization_unit_enrolled_ + epc_randomization_unit_has_resid,data=training, size=3,linear.output=F,maxiter= 200, act.fct = "logistic" )
nn
nnbest
plot(nn)
plot(nnbest)
...
predictedclasses = predict(nnbest,training,type="class")
xtab = table(truth_values_training,predictedclasses)
confusionMatrix(xtab)
auc(as.numeric(truth_values_training),as.numeric(predictedclasses))
plot(roc(as.numeric(truth_values_training),as.numeric(predictedclasses)),print.auc=TRUE)

#classification accuracy testing
predictedclasses = predict(nnbest,testing,type="class")
xtab = table(truth_values_testing,predictedclasses)
confusionMatrix(xtab)

auc(as.numeric(truth_values_testing),as.numeric(predictedclasses))
plot(roc(as.numeric(truth_values_testing),as.numeric(predictedclasses)),print.auc=TRUE)
...
nn <- nnet(lol ~ mean_rxbaseline + epc_randomization_unit_ehr_imple + nos_pct1 + epc_randomization_unit_enrolled_ + epc_randomization_unit_has_resid,data=training, size=5,linear.output=F,softmax=TRUE,act.fct = "logistic" ,entropy=TRUE,maxit=300)
